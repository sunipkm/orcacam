...
DCAM_DECLARE_BEGIN(enum, DCAMERR){
    ...
    DCAMERR_NORESOURCE = 0x80000201,    /* not enough resource
                                           except memory */
    ...
    DCAMERR_FAILEDWRITEDATA  = 0x84001003, /* DCAMREC failed
                                              to write the data */
    ...
    DCAMERR_IMAGE_NEWRUNTIMEREQUIRED = 0x84003002, /* version of image header is newer than version thatused DCAM supports */
    ...
    DCAMERR_NOTIMPLEMENT = 0x80000f02, /* not implemented */
    ...
    DCAMERR_CANCELRELOADFRAME = 0x80000f0b, /* cancel to re-load frame from
                       hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE */

    DCAMERR_DEVICEINITIALIZING = 0xb0000001, /* device initializing */
    ...
    DCAMERR_MISSPROP_TRIGGERSOURCE =
        0xE0100110, /* the trigger mode is internal or
                       syncreadout on using device memory. */
...
} DCAM_DECLARE_END(DCAMBUF_ATTACHKIND)
...
DCAM_DECLARE_BEGIN(enum, DCAMDATA_KIND){
    DCAMDATA_KIND__REGION = 0x00000001, /** Region */
    DCAMDATA_KIND__LUT    = 0x00000002, /** Lut */
    DCAMDATA_KIND__NONE   = 0x00000000, /** None */
...
DCAM_DECLARE_BEGIN(struct, DCAMDEV_CAPABILITY_FRAMEOPTION)
{
    ...
    hdr; // [in] size:  size of this structure
         ...
         // [out]capflag: available DCAMBUF_PROCTYPE__* flags in
         // current condition. [in] kind:  0 reserved
...
DCAM_DECLARE_BEGIN(struct, DCAM_METADATATEXTBLOCK)
{
    ...
    //// [in] pointer for filling each text size of METADATA
    /// bytesperunit / [in] max bytes per unit for filling each METADATA
    /// textcodepage / [in] pointer for filling each text codepage of
    /// METADATA

    // dcamrec_lockmetadatablock()
    // buf
    //// [out] return DCAM internal pointer of userdatatext block unitsizes
    //// [out] return DCAM internal array pointer of each size
    /// bytesperunit / [out] max bytes per unit which is set at DCAMREC_OPEN
    /// textcodepage / [out] return DCAM internal array pointer of each
    /// codepage
}
...
DCAM_DECLARE_BEGIN(struct, DCAM_METADATABINBLOCK)
{
    ...
    //// [in] pointer for filling each bin size of METADATA
    /// bytesperunit / [in] max bytes per unit for filling each METADATA
...
DCAM_DECLARE_BEGIN(struct, DCAM_METADATABLOCK)
{
    ...
    //// [in] pointer for filling each unit size of METADATA
    /// bytesperunit / [in] max bytes per unit for filling each METADATA
